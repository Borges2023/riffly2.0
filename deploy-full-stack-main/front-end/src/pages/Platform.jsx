...
const Platform = () => {
  ...
  const songs = useMemo(() => getSongs().slice(0, 24), []);
  const artists = useMemo(() => getArtists().slice(0, 12), []);
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/platform/overview`)
  ...
  const activeAudio = activeSong?.audio || activeSong?.audioFallback || "";
    ...
    <main className={`platform platform--${theme}`}>
      ...
      <section className="platform-player">
        ...
        <div className="platform-player__main">
          ...
          <audio ref={audioRef} src={activeAudio} onEnded={() => setIsPlaying(false)} />
      ...
      <section className="platform-grid">
        <div className="platform-panel platform-panel--wide">
          <div className="platform-panel__header">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar musica, artista ou genero" />
          </div>
          <div className="platform-song-list">
            {filteredSongs.slice(0, 10).map((song, index) => (
              <button type="button" className="platform-song-row" key={song.id} onClick={() => playSong(song)}>
        ...
        <div className="platform-panel">
          ...
          <div className="platform-recs">
            {recommendations.map((song) => (
              <button type="button" key={song.id} onClick={() => playSong(song)}>
        ...
        <div className="platform-panel">
          ...
          <div className="platform-artists">
            {artists.slice(0, 5).map((artist) => (
              <button type="button" key={artist.id} onClick={() => followArtist(artist)}>
